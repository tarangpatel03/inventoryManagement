import { BAErrorToast } from './toastShow';
...
export const ErrorHandler = (Error: ErrorType | any) => {
  if (Error?.data?.message) {
    BAErrorToast(Error?.data.message);
  } else if (Error?.status && httpStatusCodes[Error.status]) {
    BAErrorToast(httpStatusCodes[Error.status]);
  } else if (Error?.code && firebaseErrorCodes[Error.code]) {
    BAErrorToast(firebaseErrorCodes[Error.code]);
  } else if (Error?.message) {
    BAErrorToast(Error?.message);
  } else if (Error?.message === 'No internet connection') {
  } else {
    BAErrorToast('Something went wrong, Please try again later.');
  }
};
